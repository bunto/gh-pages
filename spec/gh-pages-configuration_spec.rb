...
describe(GitHubPages::Configuration) do
  ...
  let(:configuration) { Bunto.configuration(test_config) }
  let(:site)          { Bunto::Site.new(configuration) }
  let(:effective_config) { described_class.effective_config(site.config) }
  before(:each) { ENV.delete("DISABLE_WHITELIST") }
  before(:each) { ENV["BUNTO_ENV"] = "test" }
  ...
  context "#effective_config" do
    ...
    it "sets default gems" do
      expect(effective_config["gems"]).to include("bunto-coffeescript")
    end

    it "lets the user specify additional gems" do
      expect(effective_config["gems"]).to include("bunto-sitemap")
    end
  ...
  context "#set being called via the hook" do
    ...
    it "sets default gems" do
      expect(site.config["gems"]).to include("bunto-coffeescript")
    end

    it "lets the user specify additional gems" do
      expect(site.config["gems"]).to include("bunto-sitemap")
    end
  ...
  context "plugins" do
    ...
    context "in development" do
      before { ENV["BUNTO_ENV"] = "development" }

      context "without the DISABLE_WHITELIST flag" do
        it "doesn't include additional whitelisted plugins" do
          expect(site.config["whitelist"]).not_to include("bunto_test_plugin_malicious")
        end
      ...
      context "with the DISABLE_WHITELIST flag" do
        ...
        it "includes additional plugins in the whitelist" do
          expect(site.config["whitelist"]).to include("bunto_test_plugin_malicious")
        end
    ...
    context "in production" do
      before { ENV["BUNTO_ENV"] = "production" }
